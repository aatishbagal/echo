...
private:
    ...
    std::string generatePythonScript(const std::string& deviceName, const std::string& username) {
        std::ostringstream manufacturerData; manufacturerData << "dbus.Array([dbus.Byte(0x11)"; for (unsigned char c : username) { manufacturerData << ", dbus.Byte(" << static_cast<int>(c) << ")"; } manufacturerData << "], signature='y')"; std::string manufacturerDataStr = manufacturerData.str(); std::ostringstream script; script << R"(#!/usr/bin/env python3
...
class Advertisement(dbus.service.Object):
    ...
    def __init__(self,bus,index,ad_type):
        self.path=self.PATH_BASE+str(index);self.bus=bus;self.ad_type=ad_type
        self.service_uuids=['F47B5E2D-4A9E-4C5A-9B3F-8E1D2C3A4B5C']
        self.manufacturer_data=dbus.Dictionary({dbus.UInt16(0xFFFF):)"; script << manufacturerDataStr; script << R"(},signature='qv')
        self.local_name=')"; script << deviceName; script << R"('
        dbus.service.Object.__init__(self,bus,self.path)
    def get_properties(self):
        p={'Type':self.ad_type,'ServiceUUIDs':dbus.Array(self.service_uuids,signature='s'),'LocalName':dbus.String(self.local_name),'ManufacturerData':self.manufacturer_data}
        return {LE_ADVERTISEMENT_IFACE:p}
...
class Application(dbus.service.Object):
    def __init__(self,bus,path):self.path=path;dbus.service.Object.__init__(self,bus,path)
    def get_path(self):return dbus.ObjectPath(self.path)
class Service(dbus.service.Object):
    def __init__(self,bus,index,uuid,primary):self.path=f"/org/bluez/echo/service{index}";self.bus=bus;self.uuid=uuid;self.primary=primary;self.chars=[];dbus.service.Object.__init__(self,bus,self.path)
    def get_path(self):return dbus.ObjectPath(self.path)
    def add_characteristic(self,ch):self.chars.append(ch)
    @dbus.service.method(DBUS_PROP_IFACE,in_signature='s',out_signature='a{sv}')
    def GetAll(self,iface):return {'UUID':self.uuid,'Primary':self.primary,'Characteristics':[c.get_path() for c in self.chars]}
class Characteristic(dbus.service.Object):
    def __init__(self,bus,service,index,uuid,flags,socket_sender):self.path=f"{service.get_path()}/char{index}";self.bus=bus;self.service=service;self.uuid=uuid;self.flags=flags;self.socket_sender=socket_sender;dbus.service.Object.__init__(self,bus,self.path)
    def get_path(self):return dbus.ObjectPath(self.path)
    @dbus.service.method(DBUS_PROP_IFACE,in_signature='s',out_signature='a{sv}')
    def GetAll(self,iface):return {'UUID':self.uuid,'Service':self.service.get_path(),'Flags':self.flags}
    @dbus.service.method(GATT_CHRC_IFACE,in_signature='aya{sv}',out_signature='')
    def WriteValue(self,value,options):
        try:
            self.socket_sender(bytes(value))
...
def main():
    ...
    service=Service(bus,0,'F47B5E2D-4A9E-4C5A-9B3F-8E1D2C3A4B5C',dbus.Boolean(1))
    tx=Characteristic(bus,service,0,'8E9B7A4C-2D5F-4B6A-9C3E-1F8D7B2A5C4E',['write','write-without-response'],socket_sender)
    service.add_characteristic(tx)
    mainloop=GLib.MainLoop();GLib.timeout_add_seconds(1,accept_conn)
    gatt_manager.RegisterApplication(service.get_path(),{},reply_handler=lambda:None,error_handler=lambda e:None)
